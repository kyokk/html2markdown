...
class HTML_Parser
{
     ...
     * @return boolean
     */
    private static function _hasParentCode(DOMNode $node)
    {
     ...
     * @return DOMNode
     */
    private function _convertChildren(DOMNode $node)
    {
        if (self::_hasParentCode($node))
            return;
        if ($node->hasChildNodes()) {
            for ($length = $node->childNodes->length, $i = 0; $i < $length; $i++) {
                $child = $node->childNodes->item($i);
                $this->_convertChildren($child);
            }
        }
        $this->_convertToMarkdown($node);
    }
    ...
    public function getMarkdown()
    {
        ...
        $markdown = preg_replace("/<head>.+<\/head>/", "", $markdown);
     ...
     * @param DOMNode $node
     */
    private function _convertToMarkdown(DOMNode $node)
    {
        ...
        switch ($tag) {
            ...
            case "pre":
                ...
                break;
            case "h1":
                $markdown = $this->_convertHeader("h1", $value);
                break;
            case "h2":
                $markdown = $this->_convertHeader("h2", $value);
                break;
            case "h3":
                $markdown = "### " . $value . HTML2MD_NEWLINE . HTML2MD_NEWLINE;
                break;
     ...
     *  e.g.
     *
     *     <img src="/path/img.jpg" alt="blah" title="Title" />
     *
     * becomes
     *
     ...
     * @return string
     */
    private function _convertImage(DOMNode $node)
    {
     ...
     * e.g.
     *
     *     <a href="http://modnerd.com" title="Title">Modern Nerd</a>
     *
     * becomes
     *
     *     [Modern Nerd](http://modnerd.com "Title")
     *
     *
     * @param DOMNode $node
     * @return string
     */
    private function _convertAnchor(DOMNode $node)
    {
     ...
     * @return string
     */
    private function _convertList(DOMNode $node)
    {
        ...
        if ($list_type == "ul") {
            ...
            $number   = $this->_getListPosition($node);
     ...
     * @return string
     */
    private function _convertCode(DOMNode $node)
    {
     ...
     * @return int
     */
    private function _getListPosition(DOMNode $node)
    {
